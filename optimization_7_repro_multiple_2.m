...
Temp=time_struct_target.table.data(:,6); % do we have to include temp?
...
%     location_modification=[currentloc,'\',mechanism{1},'\',directory,'\',fuel_sim{1},'\',class_numb_text{k},'\',num2str(j)];
    ...
    temp_modification(:,j)=time_struct_modification.table.data(:,6);
    time_modification(:,j)=time_struct_modification.table.data(:,10);
%     end
    
end
temp_modification = temp_modification(:,range);
time_modification = time_modification(:,range);
...
% for k = 1 : 1
    ...
    Temp_current = Temp(k);
    time_current = time_modification(:,k);
...
%     end
    ...
    coefsTotal =[coefsTotal coefs.(class_numb_text{n})(k,:)];
    end
    
    
    
    prediction{:,k}=totalM*coefsTotal'-d;
