...
for m = 1 : numbOfPressure
    ...
    % location_modification=[currentloc,'\',mechanism{1},'_','modify','_',date{1},'\',num2str(j)];
...
%     time_modification{j}.data=time_struct_modification.table.data(:,10);
%     classnumb_text{k}.(time_modification(:,j))=time_struct_modification.table.data(:,10);

%     time_modification =ones(3,25);
%     class.(time_modification(1:3,j))=time_struct_modification.table.data(:,10);
%     field = classnumb_text{k};
%     value(:,j) = time_struct_modification.table.data(:,10);
%     sensitivity=struct(field,value);
...
%change the normalize contant log10?e.x. log10^3 
    ...
    % p_tempPoints_gr =[1, locs2, locs,length(d_log_tk1)];
    p_tempPoints_gr =[locs2, locs];
    % temp_p_tempPoints_gr=[temp(1),temp(locs2),temp(locs), temp(length(d_log_tk1))];
